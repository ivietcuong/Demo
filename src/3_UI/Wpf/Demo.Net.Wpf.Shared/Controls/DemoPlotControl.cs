...
namespace Demo.Net.Wpf.Shared.Controls
{
    ...
    public class DemoPlotControl : Control
    {
        ...
        public override void OnApplyTemplate()
        {
            ...
            var yLeft = new LinearAxis
            {
                TickStyle = TickStyle.None,
                TextColor = OxyColor.FromRgb(93, 93, 93),
                TitleColor = OxyColor.FromRgb(93, 93, 93),
                Position = AxisPosition.Left,
                MajorGridlineStyle = LineStyle.Solid,
                Title = $"Intensity"
            };
            var yLeftKey = Guid.NewGuid();
            yLeft.Key = $"{yLeftKey}";
            _plotView.Model.Axes.Add(yLeft);
            ...
            var xKey = Guid.NewGuid();
            x.Key = $"{xKey}";
            _plotView.Model.Axes.Add(x);
            ...
            Points?.ForEach(p => _lineSeries.Points.Add(new DataPoint(p.X, p.Y)));
        ...
        private static void OnPointsChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            ...
            control._lineSeries.Points.AddRange(((List<DemoCore.Point>)e.NewValue).Select(p => new DataPoint(p.X, p.Y)));
        }
